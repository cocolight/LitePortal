    ...
    db.get('SELECT COUNT(*) as count FROM users', (err, row) => {
        ...
        db.run('INSERT INTO users (username) VALUES (?)', ['guest'], function(err) {
            ...
            db.serialize(() => {
                ...
                const stmt = db.prepare('INSERT INTO links (user_id, link_id, name, online_icon, text_icon, upload_icon, icon_type, int_url, ext_url, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
                
                testLinks.forEach(link => {
                    stmt.run([guestUserId, link.id, link.name, link.icon, '', '', 'online_icon', link.int, link.ext, link.desc], (err) => {
